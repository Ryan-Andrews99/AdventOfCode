...
export const isRangeOverlap = ([start1, end1]: number[], [start2, end2]: number[]): boolean => end1 >= start2 && end2 >= start1

export const mergeRanges = ([start1, end1]: number[], [start2, end2]: number[]): number[] => [
...
export const getAnswerPart2 = (filepath: string) => {
    ...
    const sortedRanges = ranges.sort(([start1], [start2]) => start1 > start2 ? 1 : -1)
    return mergeAllRanges(sortedRanges).map(([start, end]) => end - start + 1).reduce((s, i) => s += i)
};
